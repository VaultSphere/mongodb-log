...
真实样本 `/Users/lhp/Desktop/metrics.2026-01-13T08-36-09Z-00000` 大小为 10,485,242 字节，包含 96 个 block、2,082 个指标和 28,800 个采样点，逻辑数值约 5,996 万个。旧实现把它完整展开为 `Map<String, List<Long>>` 后需要约 1.5 GB Java 堆。
...
- 分组规则参考 MetricsAnalyze：默认使用指标路径的父路径；`serverStatus/` 前缀只从组名中移除，不改变指标真实路径。
- WiredTiger cache／capacity／transaction 和 systemMetrics netstat 的特殊路径按参考实现归入更细的固定前缀组。
...
使用 `/Users/lhp/Desktop/metrics.2026-01-13T08-36-09Z-00000`：
